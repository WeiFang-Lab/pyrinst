    ...
    def __call__(self, x: NDArray, task: Task):
        ...
        if task == Task.FREQ:
            if self.hess_dx is None:
                ...
                res = self.potential(x, task - 1)
                res[2] = hess_from_energy(x, lambda y: self.potential(y, task - 1)[1], self.dx)
            else:
                res = self.potential(x, task - 2)
                res[1] = grad_from_energy(x, lambda y: self.potential(y, task - 2)[0], self.dx)
                res[2] = hess_from_grad(x, lambda y: self.potential(y, task - 2)[1], self.dx)
            ...
            if self.grad_dx is None:
                ...
                res = self.potential(x, task - 1)
                res[1] = grad_from_energy(x, lambda y: self.potential(y, task - 1)[0], self.dx)
            res[1] = res[1][: -self.n_fix]
        else:
            res = self.potential(x, task)
