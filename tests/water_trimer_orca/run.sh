...
optimize.py -P orca -F header.txt --workingdir beads -g 3e-4 --maxstep 0.1 --mode min min_guess.xyz -o min_opt.xyz --runcmd ${ORCA} > min_opt.out
optimize.py -P orca -F header.txt --workingdir beads -g 3e-4 --maxstep 0.1 --mode ts ts_guess.xyz -o ts_opt.xyz --runcmd ${ORCA} -l min_opt.pkl > ts_opt.out
optimize.py -P orca -F header.txt --workingdir beads -g 3e-4 --maxstep 0.1 --mode inst TSopt.pkl -T $T -N $N -o inst_$N.xyz -s 0.2 -F header --runcmd ${ORCA} > inst_$N.out
